...
public final class RedisBungee extends Plugin {
        ...
        if (pool != null) {
            try (Jedis tmpRsc = pool.getResource()) {
                ...
                if (uuidCacheSize > 750000)
                    getLogger()
                            .info("Looks like you have a really big UUID cache! Run https://www.spigotmc.org/resources/redisbungeecleaner.8505/ as soon as possible.");
            }
    ...
    private void loadConfig() throws IOException, JedisConnectionException {
        ...
        if (!file.exists()) {
            file.createNewFile();
            try (InputStream in = getResourceAsStream("example_config.yml"); OutputStream out = new FileOutputStream(file)) {
